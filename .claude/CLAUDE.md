...
**Design**: `design-convert` (spec management, visual→code convert — sketch/wireframe/Figma/Canva/URL → high-fi code, section-scoped partial builds, and content-fill via `--content` — and game `.tscn` codegen) feeds `design-ship` — auto-mode build→content→check as one flow (Build lane / web); `design-tokens` for tokens/motion packs; dev-track counterpart: `dev-ship`
...
- Before tagging a release: run `python3 scripts/check-handoff.py`, `python3 scripts/check-dashboard-writers.py`, `python3 scripts/check-no-project-commit.py`, `python3 scripts/check-backlog-reads.py`, `python3 scripts/check-task-markers.py`, `node scripts/check-context-load.js`, `node scripts/check-req-tags.js`, `node scripts/check-card-provenance.js`, `node scripts/check-script-paths.js`, and `bash scripts/tests/run.sh` — all must exit 0. (`run.sh` is bash; on Windows run via WSL. `check-req-tags.js` self-tests via fixtures when run with no args — claude-config itself has no `.project/` tree to scan; pass `--project <dir>` to check a real dev-ship project for dangling REQ-tags.)

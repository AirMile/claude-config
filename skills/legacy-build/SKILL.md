...
Load the `.hf-*` class system from `references/hf-system.css` and inject it as a `<style>` block in the `<head>` of preview.html.

The system provides utility classes for: layout, spacing, colors (from THEME.md vars), typography, borders, shadows, effects, buttons, inputs, cards, badges, avatars, and component markers (`[data-hifi="active"|"done"]`).

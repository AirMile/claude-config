...
**Ship offload passes `type TWEAK` explicitly — conditionally.** When `/dev-ship`'s manual/verify rounds offload a tweak-class finding that fits `shared/TWEAK-DISCIPLINE.md § Size gate` (`dev-ship/references/phase-3-manual-finalize.md § Offload flush`), the invocation states `type TWEAK` + `depends on {feature}` up front — this overrides row-based inference below (row 1 of the WEB table still applies to a plain user-typed "tweak" description with no explicit type). The same offload flush deliberately **withholds** that hint when its own size-gate judgment finds the finding exceeds the gate (net-new surface, >3 files, new test surface, architecture) — that call falls through to the normal rows below like any other description, typically landing on `CHANGE` or the `FEATURE` default instead. A net-new-capability finding (`manual-interview-walkthrough.md § Step F`) is size-gate criterion 1 by definition, so it never carries the hint either.

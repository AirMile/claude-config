...
Loaded only for `/frontend-check` without argument (`$BATCH_MODE = true`). Drives the entire
...
lenses; frontend-check scans drive a single shared Playwright browser, fixed screenshot paths
...
4. **Completion table:**
   ...
   Next: /frontend-design → next page · /project-backlog → revise scope
   ═════════════════════════════════════════════════════════════
   ```

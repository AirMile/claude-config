...
- `tests.qualityVerdict` — **(game-pipeline only)** aggregate verdict on test quality, rendered as the first output line. game-ship's verify phase writes a 2-way form `STRONG` | `WEAK` based on PASS-ratio + test-gap observations (GUT has no mutation runner), schema `{ verdict, ranAt, passRatio, testGapCount }`.
...
| `/game-ship` (define phase)   | Creates feature.json (same as dev-ship's define phase + clarifications, game-specific design fields)                                                                   | PHASE 4  |
| `/game-ship` (build phase)    | Enriches: build, tests.checklist (playtest items), requirements. Reads clarifications as constraints                                                                   | PHASE 5  |
| `/game-ship` (verify phase)   | Enriches: tests (incl. verificationCheckpoint), requirements status, observations                                                                                      | PHASE 6  |
| `/game-ship` (refactor phase) | Enriches: refactor, status → DONE                                                                                                                                      | PHASE 5  |

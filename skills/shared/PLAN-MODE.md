...
Inline gates that call `EnterPlanMode` without the full Entry section (documented at the gate): `dev-ship (define phase)` and `game-ship (define phase)` PHASE 0 Step 4b (the plan-approval gate at the end of define — accept starts build, reject returns to the interview to revise); `dev-ship (verify phase)` (`references/fix-loop.md § Plan-mode gate`), `dev-ship (build phase)` PHASE 2b and `game-ship (build phase)` PHASE 3a (regression-not-caused-by-build path).

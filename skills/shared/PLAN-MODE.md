...
Inline gates that call `EnterPlanMode` without the full Entry section (documented at the gate): `dev-ship (verify phase)` (`references/fix-loop.md § Plan-mode gate`), `dev-ship (build phase)` PHASE 2b and `game-ship (build phase)` PHASE 3a (regression-not-caused-by-build path). (The `*-ship (define phase)` gate is **not** here anymore — it is now the exit of the full-phase define plan mode above, not a standalone inline `EnterPlanMode` at Step 4b.)

...
- **Lifecycle**: `TODO → shipped` directly — a TWEAK card never enters `DEFINED`/`DOING`/`DONE` and never runs through a ship pipeline. `/dev-tweak {card-name}` executes it on `main` (no worktree) and flips it straight to `shipped: true` + archive on completion (`shared/TWEAK-DISCIPLINE.md § Card pickup`).
  - **Escalation exception (hand off)**: when a tweak run escalates and hands the card off to `/dev-ship` (`shared/TWEAK-DISCIPLINE.md § Escalation gate` (b)), the card leaves the TWEAK lifecycle instead — `dev-ship`'s define phase overwrites its `type` away from `TWEAK` in the same write that sets `status: "DEFINED"` (`dev-ship/references/dev-define/references/phase4-sync.md` § TWEAK promotion), so it never sits as `type: "TWEAK"` + `status: "DEFINED"`. From that point it is an ordinary feature card and follows the normal `TODO → DEFINED → DOING → DONE → shipped` track.
  - **Escalation exception (park)**: when a tweak run escalates and the user parks it instead (`shared/TWEAK-DISCIPLINE.md § Escalation gate` (a)), the tweak run itself overwrites `type` away from `TWEAK` to the type inferred from the card's description, appends escalation provenance to `description`, and clears any board-set `transition` — `status` stays `TODO`. This also takes the card out of the TWEAK lifecycle (it renders in the board's ordinary "To do" section, not the "Tweaks" lane) without waiting for a ship pipeline to touch it; a later `/dev-ship` define pass may reclassify `type` again once the card is actually picked up.

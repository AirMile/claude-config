...
Universal static-analysis rules removed from the runtime audit (now `/design-ship`'s check phase, runtime-only) and promoted to generation-time bans. Loaded by Convert PHASE 2 when `design.banPacks[]` contains the pack name.

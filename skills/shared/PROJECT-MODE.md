...
dev-build, dev-define, dev-debug, dev-learn, dev-owasp, core-commit (reads `commitConvention`, not `mode`), core-audit, core-edit, core-create, core-delete, core-rewrite, core-write, core-update, core-bootstrap.
...
**Important for strike-edge and other projects:** if `backlog.html` was generated before the toggle feature shipped, it won't show the ⚙ button. Re-open the backlog via `/project-backlog` (or the serve-backlog viewer) to get a fresh render from the current template. Skills always read `project.json` — not the HTML — so the file is the correct source of truth regardless.

...
//   /{project}/canvas                         → list canvases (index.json)
//   /{project}/canvas/<slug>                  → serve Excalidraw canvas
//   /{project}/canvas/<slug>.json             → GET/PUT raw .excalidraw scene
//   /{project}/canvas/<slug>/library          → merged built-in + project lib
//   /{project}/canvas/create                  → POST: create new canvas {slug}
//   /{project}/canvas/<slug>/export           → POST: save frame PNG
...
http
  .createServer(function (req, res) {
    ...
    if (parts.length >= 1) {
      ...
      // ── Canvas routes: /{project}/canvas/* ──

      if (parts[1] === "canvas") {
        // Canvas index: GET /{project}/canvas
        if (req.method === "GET" && parts.length === 2) {
          const indexFile = path.join(
            projectPath,
            ".project/canvas/index.json",
          );
          let canvases = [];
          try {
            if (fs.existsSync(indexFile))
              canvases = JSON.parse(fs.readFileSync(indexFile, "utf8"));
          } catch {}
          res.writeHead(200, { "Content-Type": "application/json" });
        ...
        // Canvas JSON API: /{project}/canvas/<slug>.json  (GET or PUT)
        if (parts.length === 3 && parts[2].endsWith(".json")) {
          const slug = parts[2].slice(0, -5);
          if (slug.includes("..")) {
            res.writeHead(400);
            res.end("Invalid slug");
            return;
          }
          const canvasDir = path.join(projectPath, ".project/canvas");
          const canvasFile = path.join(canvasDir, slug + ".excalidraw");
          ...
          if (req.method === "GET") {
            ...
            try {
              const mtime = fs.statSync(canvasFile).mtimeMs;
              if (url.searchParams.get("meta") === "1") {
                res.writeHead(200, { "Content-Type": "application/json" });
                res.end(JSON.stringify({ mtime }));
                return;
              }
              const scene = JSON.parse(fs.readFileSync(canvasFile, "utf8"));
              res.writeHead(200, { "Content-Type": "application/json" });
              res.end(JSON.stringify({ scene, mtime }));
            } catch (e) {
              res.writeHead(500, { "Content-Type": "application/json" });
              res.end(JSON.stringify({ error: e.message }));
            }
            ...
            req.on("end", () => {
              try {
                ...
                try {
                  ...
                  if (entry) entry.mtime = new Date(mtime).toISOString();
                  fs.writeFileSync(
                    indexFile,
        ...
        // Canvas library: GET /{project}/canvas/<slug>/library
        if (
          req.method === "GET" &&
          parts.length === 4 &&
          parts[3] === "library"
          ...
          const libFile = path.join(
            ...
            "frontend-sketch",
            "templates",
            "lowfi-ui.excalidrawlib",
          );
          let libraryItems = [];
          try {
            if (fs.existsSync(libFile)) {
              const lib = JSON.parse(fs.readFileSync(libFile, "utf8"));
              if (lib.library) libraryItems = lib.library;
            }
          } catch {}
          // Merge project components from design.components[]
          try {
            const dashFile = path.join(projectPath, ".project/project.json");
            if (fs.existsSync(dashFile)) {
              const proj = JSON.parse(fs.readFileSync(dashFile, "utf8"));
              const components = (proj.design && proj.design.components) || [];
              for (const comp of components) {
                const seed =
                  ...
                  99999;
                const cid =
                  "proj-comp-" + comp.name.toLowerCase().replace(/\s+/g, "-");
                libraryItems.push({
                  status: "published",
                  id: cid,
                  created: 1748995200000,
                  elements: [
                    {
                      type: "rectangle",
                      id: cid + "-rect",
          ...
          res.writeHead(200, { "Content-Type": "application/json" });
          res.end(JSON.stringify({ libraryItems }));
          return;
        }

        // Canvas frame export: POST /{project}/canvas/<slug>/export?frame=<id>
        if (
          req.method === "POST" &&
          parts.length === 4 &&
          parts[3] === "export"
          ...
          req.on("end", () => {
            try {
              const { frameId, png } = JSON.parse(body);
              if (
                !frameId ||
                !png ||
                ...
                res.end(
                  JSON.stringify({
                    error: "frameId and png (base64) required",
                  }),
                );
                return;
              }
              ...
              const safeFrame = frameId.replace(/[^a-zA-Z0-9_-]/g, "_");
              const filename = slug + "-" + safeFrame + ".png";
              const outPath = path.join(wireframesDir, filename);
              ...
              res.end(
                JSON.stringify({
                  ok: true,
                  path: ".project/wireframes/" + filename,
                }),
              );
            } catch (e) {
              res.writeHead(500, { "Content-Type": "application/json" });
        ...
        // Create canvas: POST /{project}/canvas/create  body: { slug }
        if (
          req.method === "POST" &&
          parts.length === 3 &&
          parts[2] === "create"
          ...
          req.on("end", () => {
            try {
              ...
              if (!fs.existsSync(canvasDir))
                fs.mkdirSync(canvasDir, { recursive: true });
              // Idempotent — don't overwrite existing canvas
              if (!fs.existsSync(canvasFile)) {
                fs.writeFileSync(
                  canvasFile,
                  JSON.stringify(
        ...
        // Serve canvas page: GET /{project}/canvas/<slug>
        if (
          req.method === "GET" &&
          parts.length === 3 &&
          ...
          const canvasTemplatePath = path.join(
            __dirname,
            "..",
            "..",
            "frontend-sketch",
          ...
          if (!fs.existsSync(canvasTemplatePath)) {
            res.writeHead(500);
            res.end("Canvas template not found: " + canvasTemplatePath);
            return;
          }
          let pageRef = null;
          try {
            ...
            if (fs.existsSync(indexFile)) {
              ...
              const entry = index.find((e) => e.slug === slug);
              if (entry) pageRef = entry.pageRef || null;
            }
          } catch {}
          try {
            let html = fs.readFileSync(canvasTemplatePath, "utf8");
            const cfgScript = `<script>window.__canvasConfig=${JSON.stringify({ slug, project: projectDir, pageRef })};</script>`;

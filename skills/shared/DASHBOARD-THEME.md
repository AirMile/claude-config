...
`cssVars` = complete CSS variables export — **web domain** (derived; consumed by `/design-convert` and `/design-ship`)
...
`setupContext[]` = append-only log of external sources used during setup (written by `/design-tokens` Create and `/design-convert` Convert); each entry: `{source, url, fetchedAt, appliedBy}`; keyed on `appliedBy` — re-run replaces, does not duplicate

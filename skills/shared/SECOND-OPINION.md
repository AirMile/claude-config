...
2. **Auto-spawn** — the trigger fires the consult immediately, no confirmation step — do not
   ask permission, not even via `AskUserQuestion`:
...
3. **Budget** — max **1 consult per phase**, tracked durably wherever the skill already has a
   ...
   - **Per-run backstop**: max **4 consults total per run**, across all phases/rounds combined,
     ...
     4 is reserved for the heavy debug round** (`debug-round-heavy.md`; game-debug PHASE 5-9)
     specifically — consumed in temporal order, an early-run consult spree (define, then a
     fix-round) would otherwise starve the highest-payoff consult in the run. Define + the gate
     hook + one ordinary fix-round consult still leaves that reserved slot untouched; only a
     4th non-debug trigger is turned away before the heavy round is reached. **Within that
     reserved slot**, the round's own per-round budget of 1 still applies: the round's fix-plan
     hook (contested diagnosis) and its dead-end ceiling consult (fix failed) share one slot, not
     two — whichever fires first inside the round consumes it, and the other logs
     `unavailable (budget)`.
...
— shares the gate's one slot; `fix-round.md` root-cause gate; heavy debug round in
`debug-round-heavy.md` — § 5 fix-plan hook and § 8 ceiling share the round's one slot; report row)
and `dev-manual` (shares `fix-round.md`/`debug-round-heavy.md` with dev-ship, same hooks), `game-ship`
(vendored define gate + `fix-round.md` — manual sync with dev-ship's; the define-interview click is
**dev-ship only for now** — `game-define`'s vendored `phase1a-interview.md` lacks the Assumption
Block the click's precedence rule depends on, so it is not yet ported; see `CHANGELOG.md`'s entry
for this change), `game-debug` (PHASE 5 fix-plan hook; PHASE 9 fix ceiling — same shared-slot rule),

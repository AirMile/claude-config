...
3. **Budget** — max **1 consult per phase**, tracked durably wherever the skill already has a
   ...
   - **Per-run backstop**: max **4 consults total per run**, across all phases/rounds combined,
     ...
     `unavailable (budget)` and skipped — never block the run to squeeze in a 5th.
...
`dev-ship` (define gate Step 4b; `fix-round.md` root-cause gate; heavy debug ceiling in
...
dev-ship's), `game-debug` (PHASE 6 fix-strategy; PHASE 9 fix ceiling), `dev-security` (PHASE 3
triage escalation; PHASE 5 fix-strategy), `project-plan` (pre-P1 gate), `project-seed`
(seed/critique synthesis), `design-ship` (PHASE 0 direction brief), `core-orchestrate` (PHASE 2
synthesis), `dev-tweak` (lane routing, PHASE 3 fail path — session-memory budget per § Gate, no
checkpoint). The existing opus spawns in `dev-ship` (verify, ship security triage) and
`core-orchestrate` (PHASE 4 verify) are NOT consults and stay unchanged.

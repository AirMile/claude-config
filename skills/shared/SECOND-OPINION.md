...
| debug light round — after investigation, before the fix plan (dev-ship `debug-round.md § 5`)            | § 4's investigation **refuted** the round's entry hypothesis, or returned "no defect found" while the symptom persists — the round is about to design a fix on a reading that already failed once, and an independent reader of the same dossier is the cheapest correction available. Fires **after** the evidence exists, never before it (§ Deliberately excluded). Skip when the digest points at a cause: § Route sends a factual gap to Explore, not to a spar. Counterpart: **Fable**.           |
...
**The line that exclusion draws is temporal, not per-tier.** What is excluded is a consult fired
_before_ the round has evidence — there is nothing to attack yet, so the counterpart can only
speculate. A consult fired _after_ an investigation, on its digest, is a different act and is
allowed at any tier: that is what the light-round and heavy-round fix-plan rows above both do. Do
not read the exclusion as "consults belong at the ceiling"; read it as "consults need evidence
first". An unconditional per-round consult is still wrong for the same reason — most rounds end
with a digest that names a cause, and § Route sends those to Explore.

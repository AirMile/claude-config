...
**Who runs it** (every terminal append point): `/core-pull` (after its dedup-and-sync), the ship orchestrators `dev-ship` / `game-ship` / `design-ship` (PHASE 5, after ship-level extraction), `dev-ship` / `game-ship` are covered at the orchestrator level so their build/verify/refactor domain phases — including dev-ship's own in-ship debug rounds (`debug-round.md`/`debug-round-heavy.md`), which do not extract learnings themselves — do **not** each run it, `game-debug` (PHASE 10, after the per-bug pitfall append, standalone skill), `dev-tweak` / `game-tweak` (PHASE 4, after the optional 0-1 pitfall append — see `TWEAK-DISCIPLINE.md § Registration policy`), and `core-setup --mode=mature` (after the onboard write-back).

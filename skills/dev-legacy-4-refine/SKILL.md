...
**Trigger**: `/dev-legacy-4-refine` command after a feature has been implemented via /dev-legacy-2-code
...
**NOT for:**
- New features from scratch (use /dev-legacy-1-plan + /dev-legacy-2-code)
- Code quality improvements (use /dev-legacy-5-refactor)
- Security hardening (use /dev-legacy-5-refactor)
- Performance optimization (use /dev-legacy-5-refactor)
- Architecture changes (use /dev-legacy-1-plan with change mode)
- Database migrations (use /dev-legacy-1-plan with extend/change mode)
...
**If scope too large:**
```
⚠️ SCOPE TOO LARGE FOR /dev-legacy-4-refine

This modification requires:
- [reason: new migrations / new dependencies / architecture change]

Use /dev-legacy-1-plan with 'change' mode instead:
/dev-legacy-1-plan change {feature-name}
```
...
1. **Check if feature name provided with command:**
   ...
   **If user ran `/dev-legacy-4-refine {feature-name}`:**
   - Skip numbered list
   ...
   **If user ran `/dev-legacy-4-refine` without arguments:**
...
2. **List available features (only if no feature name provided):**
   ...
   - **If no features found:**
     ...
     Run /dev-legacy-1-plan + /dev-legacy-2-code first to implement a feature.
     ```
   ...
   | 3 | notifications | (no worktree) | ✗ Not implemented | Run /dev-legacy-2-code first |
...
4. **Auto-detect part/extend/change (always runs after feature selection):**
   ...
   **If no parts/extends/changes implemented:**
   ...
   Run /dev-legacy-2-code {feature-name} first.
   ```
   → EXIT skill gracefully
   ...
   - If not found:
     ...
     options:
       ...
       - label: "Eerst implementeren"
         description: "Sluit af en run /dev-legacy-2-code {name} eerst"
       - label: "Afsluiten"
         description: "Sluit /dev-legacy-4-refine af"
     ...
     **If "Eerst implementeren":** EXIT skill with message to run /dev-legacy-2-code first
   ...
   **Steps:**
   ...
   c. **If NOT in correct worktree:**
      ...
      **If "Open worktree":**
      ...
      Switch naar dat venster en run /dev-legacy-4-refine {feature-name} opnieuw.
      ```
...
**Goal:** Load existing implementation and verify scope is appropriate for /dev-legacy-4-refine.
...
1. **Load ALL files in feature folder:**
   ...
   | `02-implementation.md` | Implementation log (REQUIRED) | If /dev-legacy-2-code ran |
   | `02-tests.md` | Test plan | If /dev-legacy-2-code ran |
   | `04-refine-*.md` | Previous refinement logs | If /dev-legacy-4-refine ran before |
   | `05-refactor-*.md` | Previous refactor logs | If /dev-legacy-5-refactor ran |
...
2. **Use sequential thinking for scope analysis:**
   ...
   - Determining if /dev-legacy-4-refine appropriate
   ```
...
3. **Scope validation:**
   ...
   **If ANY criterion fails:**
   ...
   - May require /dev-legacy-1-plan later anyway

   Recommendation: Use /dev-legacy-1-plan with 'change' mode
   Command: /dev-legacy-1-plan change {feature-name}
   ```

   Use AskUserQuestion tool:
   ```
   header: "Scope Overschrijding"
   question: "Scope is te groot voor /dev-legacy-4-refine. Wat wil je doen?"
   options:
     - label: "Gebruik /dev-legacy-1-plan (Recommended)"
       description: "Sluit af en gebruik /dev-legacy-1-plan change {feature-name}"
     - label: "Toch doorgaan"
...
5. **Optional clarification (max 2 questions):**
   ...
   - If modification still unclear after 2 questions:
     ...
     - If "Use /1-plan": Exit and suggest /dev-legacy-1-plan
     - If "Exit": Exit skill
...
| **Scope** | ✓ Within /dev-legacy-4-refine limits |
...
2. **Launch 3 research agents in parallel:**
   ...
   **Prompt for each agent:**
   ```
   Research for /dev-legacy-4-refine skill - functional modification.
   ...
   - **After 3 adjustments:**
     ...
     options:
       ...
       - label: "Gebruik /1-plan"
         description: "Schakel over naar /dev-legacy-1-plan voor gedetailleerde planning"
       - label: "Explain question"
...
6. **Show next command suggestion (for options 3 and 4):**
   ...
   /dev-legacy-5-refactor {feature-name}

   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

   💡 Use /dev-legacy-5-refactor for: code quality, performance, security improvements
...
- Better to redirect to /dev-legacy-1-plan than attempt too-large modification
...
The skill uses these specialized agents (same as /dev-legacy-1-plan):
...
### Scope Too Large
```
⚠️ SCOPE EXCEEDS /dev-legacy-4-refine LIMITS

This modification requires:
- {specific issue}

Recommendation: /dev-legacy-1-plan change {feature}
```

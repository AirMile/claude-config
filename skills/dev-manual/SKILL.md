---
name: dev-manual
description: Run a shipped feature's manual round — verify, debug, refactor, merge. Use with /dev-manual.
...
0 below): call `TaskCreate` with these 4 items, `status` per the routing branch that fired (never
all `pending` — seed the ones the checkpoint already shows as done as `completed` directly):
...
or go straight to `references/debug-round-heavy.md` (non-ledger entry) for a repeated build/verify

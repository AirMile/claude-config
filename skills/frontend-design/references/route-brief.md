...
- **Animation pack**: `{pack}` — springs: {spring tokens} · choreography: {active compositions}. Map each interactive component to its choreography token (onHover / onPress / onEnter / onSuccess / onError). Reference `skills/frontend-animations/references/choreography.md` for composition specs.

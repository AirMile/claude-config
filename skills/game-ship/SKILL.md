...
- **Two human touchpoints.** PHASE 0 (define only — technique passes are auto-derived) up front, and
  PHASE 3 (the live playtest) mid-run. Everything else runs hands-off.
- **The playtest is one flow, not two paths.** PHASE 3 either has MANUAL playtest items or falls
  through to just the merge. The **playtest classification** computed in PHASE 0 (COVERED=GUT vs
  MANUAL=playtest) is an **advisory estimate**; AGENT 2's returned `remainingManualItems` is
...
context. Any interruption becomes a resumable pause: re-invoking `/game-ship {feature}` detects the
checkpoint (PHASE 0) and offers Resume/Restart/Inspect. Only the main chat writes it (subagents
never touch it — contract rule 1). Use `pipeline: "game"` in the checkpoint.

1. PHASE 0: Define + Classify + Auto-derive technique plan
2. PHASE 1: Build (AGENT 1)
3. PHASE 2: GUT auto-verify (AGENT 2)
4. PHASE 3: Human playtest + Finalize/merge
5. PHASE 4: Refactor (AGENT 3)
6. PHASE 5: Report
...
> and update the checkpoint `phase: "PHASE 3"` (the playtest resumes from
...
> `remainingManualItems` (non-empty) → run the live playtest walkthrough then finalize. If empty →
> execute Step 1 (enter worktree) then Step 3 (Completion + Finalize, **both** items — the
> completion-sync DONE write and the finalize/merge); skip only Step 2 (the playtest walkthrough).
...
empty), finalize = merge + remove worktree via the reused `game-verify` finalize flow. On a playtest
FAIL, feedback is categorized TESTABLE / MEASURABLE / SUBJECTIVE → bounded fix loops (max 2-3
rounds), then the reference's routing question decides: an isolated background fix agent (re-test),
interactive `/game-debug`, or stop — no finalize/refactor until the failed items pass. After fixes,
re-run the GUT regression suite before finalize.

### PHASE 4: Refactor (AGENT 3) — Workflow 2
...
> learnings), **write the pointer + slice file** under `.project/session/ship-prompts/`
...
> clear `activeWorkflow`/`workflowRunId`/`prompts`, merge `refactor` into `results`,
> checkpoint `phase: "PHASE 5"`, `completedPhases += ["PHASE 4"]`.

The workflow runs AGENT 3 (post-merge, on main) running `game-refactor` on this single feature with
the **auto-derived lenses**, applying only high-confidence findings, GUT test-guarded (revert-on-red)
— no pre-build intensity toggle. `refactor.status: failed` is **non-fatal** (the feature is already
merged) — surface it for manual follow-up. **Re-read `.project/` from disk** before continuing.

...
One background agent that runs the autonomous PHASE 1–4 stretch — build, GUT auto-verify, the
no-playtest PHASE 3 completion, refactor, and finalize — so the define-heavy main chat wakes only
once instead of re-reading its whole context at every workflow boundary.

The full agent instruction body is the **static** file
`.claude/skills/game-ship/references/prompts/orchestrator.md` — the agent reads it itself. The main
...
(write points 2, 3, 4, and the PHASE 3 no-playtest completion patch); the `active-{feature}.json`
live signal during its run.

**What it never does**: user interaction (`AskUserQuestion`/`EnterPlanMode`/`ExitPlanMode`),
`TaskCreate`/`TaskUpdate`, opening the interactive game window (`mcp__godot-mcp__run_project` —
no display), ship-level learning extraction (PHASE 5, main chat only), checkpoint deletion (write
point 5 is always the main chat's).
...
its playtest round, with no build/verify paths — the checkpoint routes AGENT O straight to
PHASE 4).

```
Agent(subagent_type: "general-purpose", model: "sonnet", run_in_background: true, prompt: `
Read \`.claude/skills/game-ship/references/prompts/orchestrator.md\` — it is your full instruction set.
...
(build, adversarial GUT verify, refactor) lives in the workflows it launches.

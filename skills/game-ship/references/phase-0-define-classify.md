...
net-new surface)` into the PHASE 5 report. On a non-tweak-sized draft this note doesn't apply —
   ...
   - **Abort (park the whole feature — a decision _not to build_, distinct from Reject)** → the
     ...
     revise-loop. **Read `.claude/skills/game-ship/references/define-park.md § 1` and follow it — on
     ...
     **Do not resolve the outcome here.** "Park it, the card stays `TODO`" is one of the four, not
     ...
     board badge and a spurious resume into `PHASE 0 · define` on the next `/game-ship {feature}`):
...
"shipping"` from the backlog card) — no `note` this time, and `status` stays at its prior value
     ...
     next, not a later `/game-ship` re-invoke). No `feature.json` was written — the draft

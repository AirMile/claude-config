...
**Dual reader**: this file is read by the main chat on both paths — the playtest-items path (below),
and the no-playtest path (`references/orchestration.md § 4`), which runs only **Step 1** + **Step 3**,
never Step 2 (no game window launch, no walkthrough — this route only fires when there's nothing to
show a human) and never the routing sections below (those assume playtest items).
...
inline-vs-agent dispatch per group), the `ship-game-fix.js` dispatch (Sonnet), and the post-dispatch

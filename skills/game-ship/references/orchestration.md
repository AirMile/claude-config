...
and `phase-3-playtest.md`'s own resume check ("`playtest.items` shorter than
`results.verify.remainingManualItems`") both read this field directly from disk in a later session —
a summarized or missing array silently reads as "0 items remain" and mis-routes a fully-open
playtest round to completion. Then branch:

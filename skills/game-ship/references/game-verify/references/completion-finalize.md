...
1. **Reconcile the archive.** The pre-merge refactor agent wrote its completion batch in the worktree
   ...
   every feature-level postcondition `game-refactor/references/completion-batch.md` requires here,
   ...
   d. **Feature-dir moved** (`completion-batch.md § Step 3b`):
   `.project/features/archive/{shippedAt}-{name}/` exists and `.project/features/{name}/` no longer does.
   ...
   **Self-heal a half-run** (each independently — a and d fail separately):
   ...
   - Feature-dir not moved → run `completion-batch.md § Step 3b`'s `mv` now (idempotent):

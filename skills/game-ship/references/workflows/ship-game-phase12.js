export const meta = {
  name: "ship-game-build-verify",
  description:
    "game-ship PHASE 1+2: build the feature (sonnet/high), then adversarial GUT auto-verify in a fresh context (opus/high)",
  whenToUse:
    "Launched by the game-ship skill after PHASE 0 — not intended for standalone use.",
  phases: [
    {
      title: "Build",
      detail: "AGENT 1 — game-build in worktree (headless GUT), never merges",
      model: "sonnet",
    },
    {
      title: "Verify",
      detail:
        "AGENT 2 — game-verify COVERED GUT items, fresh context, headless",
      model: "opus",
    },
  ],
};
...
const VERIFY_SCHEMA = {
  ...
  properties: {
    ...
    qualityVerdict: {
      enum: ["STRONG", "WEAK"],
      description:
        "GUT test-quality verdict (game-verify PHASE 5d; passRatio + feedback FAILs, no mutation runner)",
    },
    passRatio: {
      type: "number",
      description: "COVERED GUT pass ratio, 0..1",
    },
    remainingManualItems: {
      type: "array",
      description:
        "MANUAL playtest items for the main-chat walkthrough (PHASE 3); empty when none",
      items: {
        type: "object",
        properties: {
          id: { type: "integer" },
          ...
          expected: {
            type: "string",
            description: "observable gameplay outcome",
          },
        },
...
if (!build || build.status !== "green") {
  // Failed build: never run verify; the worktree stays intact for /game-debug.
  return {
    status: "failed",
    failedPhase: "build",
    build: build ?? {
...
log(
  `build green: ${build.testsPass}/${build.testsTotal} GUT tests, worktree ${build.worktreePath}`,
);

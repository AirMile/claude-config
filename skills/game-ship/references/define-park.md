...
Loaded only when PARK-ESCAPE fires: during game-ship's define (Steps 2c→4, inside plan mode) the
...
so no parked-run row lingers. And the Split outcome below is NOT game-define's own split proposal
...
the backlog in memory (loaded in PHASE 0) for swap candidates per § 3 so the first option can
...
From `backlog.json#features[]`, game cards (`type` in `CORE|MECHANIC|CONTENT|POLISH|UI`), pick
the top 1–3 in this order:
...
3. **Ready POLISH batch** — ≥2 POLISH cards whose dependencies have shipped. These never swap
   into game-ship — offer them as "stop and run /game-tweak" instead.

No candidate at all → omit the Swap option in § 1.

## 4 — Cleanup (after plan-mode exit — all outcomes except "continue")

Run the four gate-Abort cleanup steps from `phase-0-define-classify.md § Step 4b Abort` verbatim
...
children pushed (dedup check first; small 1–3-file children may be `type: "POLISH"`), dependents
repointed. Then AskUserQuestion: "Continue define with {first-child} (Recommended)" → jump to
Step 1 with that child as the arg (as in Swap) / "Stop here" → show the created cards + the
reorg tip from Plain park, and stop.

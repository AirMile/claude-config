...
- **The real cache-rebuild cost was `opusplan` model switches, not idling.** (Historical — the
  ...
  `EnterPlanMode` (mirrors `dev-ship/references/phase-0-fresh-define.md:9`) landed right after the
  last mandatory pre-plan-mode write, while the main chat still held only `SKILL.md` +
  `phase-0-define-classify.md` — a cheap write. Everything expensive (context loads, the interview,
  architecture, the plan file) then accumulated **inside** plan mode, so the costly rebuild was
  concentrated at the single `ExitPlanMode` at gate exit, reading the full ~40-60k-token chat — not
  doubled on both sides. The general lesson still holds for any future model-switch gate: enter as
  early as possible in the context (right after the last write that must precede it), so the cheap
  side of the switch is the entry, not the exit. Plan mode is kept anyway — the define architecture is
  built around it as a genuine approval gate (`SKILL.md § Design`) — reviewing the plan file was worth
  that one uncached read; today it costs nothing cache-wise.

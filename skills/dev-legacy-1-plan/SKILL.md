...
**Trigger**: `/dev-legacy-1-plan` or `/dev-legacy-1-plan [task-type]` or `/dev-legacy-1-plan [description]`
...
**Triggers:**
- `/dev-legacy-1-plan` - Start workflow with task type selection
- `/dev-legacy-1-plan feature` - Start with FEATURE classification
- `/dev-legacy-1-plan extend` - Start with EXTEND classification
- `/dev-legacy-1-plan [description]` - Start with task description
  - Example: `/dev-legacy-1-plan add email notifications to orders`
  - Example: `/dev-legacy-1-plan create new authentication system`
  - Example: `/dev-legacy-1-plan modify user profile to include bio field`
...
4. **Store requirements for output:**
   ...
   - /dev-legacy-3-verify will test against these requirements
...
3. **If baseline DOES NOT EXIST:**
   ...
   Tip: Run /core-setup to generate stack baseline for future features.
   ...
   - options:
     ...
     - label: "No", description: "Exit and run /core-setup first (recommended for new projects)"
   - multiSelect: false

   **Response Handling:**
   - "Yes" → Continue to Step 1 without baseline
   - "No" → Exit and suggest running /core-setup first
...
```
Perform autonomous Context7 research for /dev-legacy-1-plan skill FASE 3.
...
**Why here:** Quality checks should happen BEFORE decomposition decision. If /thinking-analyze suggests simplifications, complexity score will be lower.
...
1. **Generate intermediate context:**
   ...
   This gives /thinking-analyze something to work with.
...
2. **Ask user for analysis preference:**
   ...
   **Response Handling:**
   - "Yes, Analyze" → Continue to step 3 (run /thinking-analyze)
   - "No, Continue" → Skip to FASE 5 with original research data

3. **If user chooses "Yes, Analyze":**

   a. **Run /thinking-analyze skill:**
   ```
   🔍 Running /thinking-analyze skill...
   ```

   b. **Execute /thinking-analyze workflow:**
   - Load intermediate context from /tmp/
   - /thinking-analyze runs full analysis:
   ...
   c. **After /thinking-analyze completes:**
   ```
   ✅ Analysis complete
   ...
   - If /thinking-analyze made improvements, research data is updated
...
5. **Key benefit:**
   ...
   **Scenario 1 - Simplification:**
   ```
   Without /thinking-analyze: Complexity 78 → PARTS (3 parts)
   With /thinking-analyze: Removed feature X, deferred Y → Complexity 55 → SINGLE_TASK
   ```

   **Scenario 2 - Risk identified:**
   ```
   /thinking-analyze: "Assumption about external API is unvalidated"
   → User adds validation part
   → Complexity increases but plan is better
   ```

**Continue to FASE 5** with validated/improved research data (if /thinking-analyze ran) or original data (if skipped)
...
```bash
python3 .claude/skills/dev-legacy-1-plan/scripts/generate_output.py \
...
Ready for implementation via /dev-legacy-2-code

Next: Run /dev-legacy-2-code {name}
```
...
Next: Run /dev-legacy-2-code {parent}
```
...
7. **Report worktree creation:**
   ...
   Or use: /dev-worktree {feature-name}
   ```
...
/dev-legacy-2-code {feature-name}
...
2. Retry /dev-legacy-1-plan to try again

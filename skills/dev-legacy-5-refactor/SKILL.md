...
**Trigger:**
- `/dev-legacy-5-refactor` command in Claude Code

**Requirements:**
- `/dev-legacy-1-plan` skill has generated context (`.workspace/features/{name}/01-intent.md` + `01-research.md`)
- `/dev-legacy-2-code` skill has generated implementation (`.workspace/features/{name}/02-implementation.md`)
- Feature code exists and tests are passing
- No blocking bugs or issues

**NOT for:**
- Fixing broken tests (use /dev-legacy-3-verify first)
- Adding new features (use /dev-legacy-1-plan then /dev-legacy-2-code)
- Architecture redesigns (requires new /dev-legacy-1-plan cycle)
- Breaking changes (requires new /dev-legacy-1-plan cycle)
...
1. **Validate features directory exists:**
   ...
   **If directory does not exist:**
   ...
   First run /dev-legacy-1-plan and /dev-legacy-2-code to implement a feature.
   ```
   → EXIT skill gracefully

2. **Check if feature name provided with command:**

   **If user ran `/dev-legacy-5-refactor {feature-name}`:**
   - Skip numbered list
   ...
   **If user ran `/dev-legacy-5-refactor` without arguments:**
...
3. **List available features (only if no feature name provided):**
   ...
   **Use AskUserQuestion tool:**
   ...
   - options: (dynamically built from scan results)
     - For each implemented feature:
       ...
       - description: "✓ Geïmplementeerd{, N parts/extends/changes}" (if applicable)
     - For each non-implemented feature:
       - label: "{feature-name}"
       - description: "✗ Niet geïmplementeerd - run /dev-legacy-2-code eerst"
   ...
   **Example options (dynamic):**
   ```
   options:
     - label: "checkout (Recommended)", description: "✓ Geïmplementeerd, 2 parts"
     - label: "user-profile", description: "✓ Geïmplementeerd"
     - label: "notifications", description: "✗ Niet geïmplementeerd - run /dev-legacy-2-code eerst"
     - label: "Uitleg vraag", description: "Leg uit wat feature selectie betekent"
   ```

   **Handle "Uitleg vraag" response:**
   ```
   📖 FEATURE SELECTIE UITLEG
   ...
   (/dev-legacy-1-plan) en geïmplementeerd (/dev-legacy-2-code).

   Features met "✓ Geïmplementeerd" kunnen worden gerefactored.
   Features met "✗ Niet geïmplementeerd" moeten eerst worden
   gebouwd met /dev-legacy-2-code.
...
4. **Handle user selection:**
   - If number: map to feature name → proceed to step 5
   - If name only: validate exists → proceed to step 5
...
5. **Auto-detect part/extend/change (always runs after feature selection):**
   ...
   **If feature HAS parts/extends/changes:**
   ...
   - For each section, check refactor status in `05-refactor.md`:
     - If section exists → `✓ Refactored`
     - If section missing → `○ Not refactored`
   ...
   | # | Name | Type | Status |
   |---|------|------|--------|
   | 1 | 01-cart-models | Part | ✓ Refactored |
   | 2 | 02-payment-backend | Part | ○ Not refactored ← NEXT |
   ...
   **Steps:**
   ...
   c. **If NOT in correct worktree:**
      ...
      **If "Open worktree":**
      ...
      Switch naar dat venster en run /dev-legacy-5-refactor {feature-name} opnieuw.
      ```
...
2. **Validate required files exist:**
   ...
   **If context file missing:**
   ...
   This file is generated by /dev-legacy-1-plan.
   First run /dev-legacy-1-plan for this feature.
   ```
   → EXIT skill gracefully
   ...
   **If implementation file missing:**
   ...
   This file is generated by /dev-legacy-2-code.
   First run /dev-legacy-2-code for this feature.
   ```
   → EXIT skill gracefully

   **If implementation file is empty or contains no file paths:**
   ```
   ❌ IMPLEMENTATION FILE EMPTY

   File contains no code files: {implementation-file-path}
   The file exists but has no file paths to refactor.
   Check if /dev-legacy-2-code was executed correctly.
   ```
   → EXIT skill gracefully
...
4. **Build pipeline files list (for scope tracking):**
   ...
   - `02-implementation.md` → files created/modified during coding
   - `04-refine.md` → files modified during refinement (if exists)
...
| File | Status |
|------|--------|
| 00-overview.md | ✓ / not present |
| 01-intent.md | ✓ |
| 01-research.md | ✓ |
| 02-implementation.md | ✓ |
| 03-verify.md | ✓ / not present |
| 04-refine.md | ✓ / not present |
   ...
   - **If expired:** Show warning and decision prompt:
     ...
     **Use AskUserQuestion tool:**
     ...
     - options:
       ...
       - label: "Stop & refresh"
         description: "Stop and run /core-setup to regenerate baseline first"
     ...
     **Handle response:**
     ...
     - If user chooses "Stop & refresh":
       - EXIT skill with message: "Run /core-setup to refresh stack baseline, then re-run /5-refactor"
...
2. **Ask user which research areas to include:**
   ...
   **Error Handling**: Zoekt naar foutafhandelingspatronen zoals retry
   ...
   ```
   → Return to research area selection

3. **Spawn agents ONLY for selected areas (max 4, min 1):**

   **If "Security" selected → spawn Agent: security-researcher**
   ...
   **If "Performance" selected → spawn Agent: performance-researcher**
...
4. **Wait for all agents to complete:**
   ...
   - **Report progress as agents complete:**
     ...
     - Show which agent just finished: `✓ security-researcher complete (coverage: X%)`
...
→ Ready for relevance evaluation.
```
...
4. **Decision logic:**
   ...
   **Step B.3: Calculate weighted final score:**
   ```
   Final Coverage = (optimist_score × 0.30) + (skeptic_usability × 0.30) + (pragmatist_score × 0.40)
   ```
   ...
   **C. additional_search (single category < 50% while others >= 75%):**
   ...
   4. **After 1 retry:**
      - If still < 50% → proceed anyway with warning in output
      - If >= 50% → proceed (with or without warning based on final score)
   ...
   **Handle response:**
   - If user chooses "Retry search" → execute additional_search logic (1 retry only, no further retries)
   - If user chooses "Continue anyway" → proceed to Phase 4 with warning, log low confidence
   - If user chooses "Stop" → EXIT skill gracefully with success message

   **After retry from revise case:**
   - If coverage >= 50% → proceed to Phase 4 (with warning if < 75%)
   - If coverage still < 50% → EXIT gracefully with message:
     ```
     ...
     ```
     → EXIT skill gracefully (no further retries to prevent infinite loop)
...
```
📊 RELEVANCE EVALUATION (Multi-Perspective)
...
| evaluate-skeptic | [Y]% → [100-Y]% | Gap → Usability |
| evaluate-pragmatist | [Z]% | Actionability score |

| Metric | Value |
|--------|-------|
| **Weighted Final Score** | [W]% |
| **Calculation** | (optimist×0.30) + (skeptic_usability×0.30) + (pragmatist×0.40) |
...
1. **Scan for known security patterns:**
   - Read `.claude/skills/dev-legacy-5-refactor/references/security-patterns.md`
   ...
   - Check for `.github/workflows/*.yml` files → scan for `${{ github.event.` in `run:` commands
   - Log all pattern matches found with file:line references

2. **Scan for DRY violations and refactoring opportunities:**
   - Read `.claude/skills/dev-legacy-5-refactor/references/refactoring-patterns.md`
...
4. **Prepare agent context:**
   ...
   Security patterns reference: [from .claude/skills/dev-legacy-5-refactor/references/security-patterns.md]
   Refactoring patterns reference: [from .claude/skills/dev-legacy-5-refactor/references/refactoring-patterns.md]
   ```
...
→ Ready for plan selection in Phase 4.5.
```
...
1. **Present plan comparison overview:**
   ...
   ```
   📝 REFACTOR PLANS - COMPARISON

   ┌─────────────────────────────────────────────────────────────────────────────┐
   │ 📊 3 PLANS AVAILABLE                                                       │
   ├─────────────────────────────────────────────────────────────────────────────┤
   │                                                                             │
   │  Plan              │ Improvements │ Risk Profile   │ Effort    │ When Best │
   │ ───────────────────┼──────────────┼────────────────┼───────────┼───────────│
   │ 1. CONSERVATIVE    │ [3-5]        │ LOW only       │ [X] min   │ Tight     │
   │    "Do no harm"    │              │                │           │ deadline  │
   │                    │              │                │           │           │
   │ 2. THOROUGH        │ [15-25]      │ LOW + MED      │ [X] hr    │ Full      │
   │    "Complete"      │              │                │           │ cleanup   │
   │                    │              │                │           │           │
   │ 3. IMPACT-FOCUSED  │ [8-12]       │ Best ROI       │ [X] hr    │ Balanced  │
   │    "Maximum ROI"   │ (recommended)│                │           │ approach  │
   │                    │              │                │           │           │
   └─────────────────────────────────────────────────────────────────────────────┘
   ...
   📋 SCOPE ANALYSIS (per plan)

   | Scope           | Conservative | Thorough | Impact-Focused |
   |-----------------|--------------|----------|----------------|
   | Pipeline files  | [N]          | [N]      | [N]            |
   | External files  | [N] ⚠️       | [N] ⚠️   | [N] ⚠️         |

   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

   💡 RECOMMENDATION: [Impact-Focused / Conservative / Thorough]
...
3. **Process user choice:**
   ...
   - **If "Impact-Focused"** → use plan-impact-focused output, proceed to step 4
   - **If "Conservative"** → use plan-conservative output, proceed to step 4
   - **If "Thorough"** → use plan-thorough output, proceed to step 4
   - **If "Bekijk details"** → show plan detail selection:
   ...
   - **If "Custom selectie"** → show improvement selection with multiSelect:
     ...
     **Use AskUserQuestion tool:**
     ...
     - options: (dynamically built from improvements)
       - For each improvement:
         ...
         - description: "{issue} → {fix} (Risk: {L/M/H}, Source: {plan})"
       - Always add last option:
         - label: "Uitleg vraag"
     ...
     **Example options (dynamic):**
     ```
     options:
       - label: "[Security] src/auth.js:45", description: "SQL injection → parameterized query (Risk: LOW, Source: Conservative)"
       - label: "[Performance] src/api.js:120", description: "N+1 query → eager loading (Risk: MED, Source: Impact-Focused)"
       - label: "[DRY] src/utils.js:30-50", description: "Duplicate code → extract function (Risk: LOW, Source: Thorough)"
       - label: "Uitleg vraag", description: "Leg uit hoe custom selectie werkt"
     ```

     **Handle "Uitleg vraag" response:**
     ```
     📖 CUSTOM SELECTIE UITLEG
     ...
     - Issue → Fix: Wat het probleem is en hoe het wordt opgelost
     ...
     ```
     → Return to custom selection

     - Build custom plan from selected improvements
     - Proceed to step 4 with custom selection

   - **If "Annuleren"** → EXIT skill gracefully with message "Refactor geannuleerd door gebruiker"
...
4. **Show selected plan detail and confirm:**
   ...
   Impact Summary:
   🔴 HIGH priority: [X] improvements (security vulnerabilities)
   🟡 MEDIUM priority: [Y] improvements (performance + DRY + error handling)
   🟢 LOW priority: [Z] improvements (code quality)
...
5. **Final confirmation:**
   ...
   **Alleen pipeline bestanden**: Past alleen verbeteringen toe in
   bestanden die bij deze feature horen (aangemaakt door /dev-legacy-2-code).
...
6. **Process final confirmation:**
   ...
   - **If "Ja, pas alles toe"** → proceed to Phase 5 with full selected plan

   - **If "Alleen pipeline bestanden"** → filter to `[PIPELINE]` improvements, proceed to Phase 5

   - **If "Selecteer specifieke verbeteringen"** → show improvement selection with multiSelect:
     ...
     **Use AskUserQuestion tool:**
     ...
     - options: (dynamically built from plan improvements)
       - For each improvement in selected plan:
         ...
         - description: "{issue} → {fix} (Effort: {S/M/L}, Risk: {L/M/H}) [{PIPELINE/EXTERNAL}]"
       - Always add last option:
         - label: "Uitleg vraag"
     ...
     **Example options (dynamic):**
     ```
     options:
       - label: "[HIGH] src/auth.js:45", description: "SQL injection → parameterized query (Effort: S, Risk: LOW) [PIPELINE]"
       - label: "[MED] src/api.js:120", description: "N+1 query → eager loading (Effort: M, Risk: MED) [PIPELINE]"
       - label: "[LOW] src/utils.js:30", description: "Magic number → named constant (Effort: S, Risk: LOW) [EXTERNAL]"
       - label: "Uitleg vraag", description: "Leg uit hoe specifieke selectie werkt"
     ```

     **Handle "Uitleg vraag" response:**
     ```
     📖 SPECIFIEKE SELECTIE UITLEG
...
2. **Evaluate results:**
   - All tests pass → Proceed to Phase 7 (Documentation)
   - Any test fails → Execute rollback
...
3. **If tests fail - Immediate Rollback:**
   ...
   **If both methods fail - Manual recovery:**
   ...
   ```
   → **STOP** - do not proceed, await manual recovery
...
5. **Execute `update_docs.py` to update project documentation:**
   ```bash
   python3 .claude/skills/dev-legacy-2-code/scripts/update_docs.py
   ```
...
| Document | Status |
|----------|--------|
| Refactor log | {refactor-output-path} ✓ |
| Feature overview | {path}/00-overview.md ✓ [Technical notes updated] |
| Timestamp | [from time MCP] |
...
✅ REFACTOR COMPLETE - READY FOR MANUAL REVIEW
```
...
7. **Show completion message:**
   ...
   ✓ /dev-legacy-1-plan - Planning
   ✓ /dev-legacy-2-code - Implementation
   ✓ /dev-legacy-3-verify - Verification
   ✓ /dev-legacy-4-refine - Refinement (if used)
   ✓ /dev-legacy-5-refactor - Refactoring
...
- Always research in priority order: Security → Performance → Quality → Error Handling
...
- If breaking change needed → exit and suggest new /dev-legacy-1-plan cycle

### Incremental Application
- Apply all approved changes in Phase 5 in priority order: Security → Performance → Quality → Error Handling
...
- **Cause:** /dev-legacy-1-plan and /dev-legacy-2-code not run yet
- **Action:** Inform user to run /dev-legacy-1-plan then /dev-legacy-2-code first
...
- **Calculate weighted score from 3 evaluation agents** (optimist×0.30 + skeptic×0.30 + pragmatist×0.40)

...
Map `$VERIFY_STATUS` (from Step 9) to backlog state. **design-ship exception:** `delete
feature.transition` never applies to `transition: "shipping"` — that is the design-ship run
marker; keep it (it is removed by design-ship PHASE 4 completion or PHASE 5 cleanup):

- `"PASS"` → `feature.status = "DOING"` + `delete feature.transition` (except `"shipping"`) + `feature.stage = "built"` + `delete feature.contentStatus` + `feature.audit.buildSmokeStatus = "PASS"` + `data.updated = today`
...
Follow [Discovery — Gap-Discovery](.claude/skills/shared/SKILL-PATTERNS.md#gap-discovery), Trigger C (Build post code-gen): scan `$GENERATED_FILES` for stub handlers and show AskUserQuestion per found gap. If no gaps: skip step.

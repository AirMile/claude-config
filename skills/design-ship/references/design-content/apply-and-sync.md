...
Remove: feature.transition       (clear the "contenting" marker) — EXCEPT "shipping":
                                 the design-ship run marker stays until PHASE 4/5 cleanup
Status: keep "DOING"             (design-ship PHASE 4 is the gate to DONE)
...
## 5.4b Worktree commit (design-ship only)

Commit the applied copy inside the worktree so AGENT 3 (check) and the PHASE 4 merge see it:

```bash
git add {modified source + i18n files}   # codebase files only — session state stays local
git commit -m "content({target}): copy toegepast op gegenereerde pagina"
```

Commit message language follows `CLAUDE.md → Language`. Skip if no files changed.

...
3. **Conflict** (Create/Update) — read `.project/project.json` → check `theme` section.
   ...
   **Setup context age** (all routes except Create): if `theme.setupContext[]` has an entry with `appliedBy` starting with `"design-tokens@"`, compute `now − fetchedAt`. If > 180 days → print one non-blocking line: `⚠ Setup context is from {date} ({n} days old). Vercel guidelines may have evolved — consider re-running Create or syncing manually.`
...
7. **Domain resolution** — resolve `$DOMAIN` per `shared/DOMAIN.md` (explicit `theme.domain` →
   infer from `stack.framework`/`language` → codebase fallback → ask). Write the resolved value
   back to `theme.domain` when inferred. `native` → print the fallback line and continue spec-only.
   → `Domain: [✓ web | ✓ game | ⚠ native (spec-only)]`
8. **Stack detection** (web domain only — skip when `$DOMAIN === "game"`) — read `package.json` → detect motion library:
   ...
   - Game domain → `$STACK_TYPE=godot` (motion tokens emit to `motion_tokens.gd`, see `references/emit-godot.md`).
...
> **Todo**: Read `.claude/skills/design-tokens/references/phase-1-action-select.md`
...
> **Todo**: Read `.claude/skills/design-tokens/references/route-fill-in.md`

#### Route: Create (New Theme)

> **Todo**: Read `.claude/skills/design-tokens/references/route-create.md`

#### Route: View

> **Todo**: Read `.claude/skills/design-tokens/references/route-view.md`

#### Token Drift Check (shared helper)

> **Todo**: Read `.claude/skills/design-tokens/references/token-drift.md`

#### Route: Extract from styleguide (PDF / image / URL)

> **Todo**: Read `.claude/skills/design-tokens/references/route-styleguide.md`

#### Routes: Update, Extract from code, Modes, Delete

> **Todo**: Read `.claude/skills/design-tokens/references/route-secondary.md`

### Motion routes

#### Route: Create / Pick Pack

> **Todo**: Read `.claude/skills/design-tokens/references/motion/route-create.md`

#### Route: Customize (axis-by-axis)

> **Todo**: Read `.claude/skills/design-tokens/references/motion/route-customize.md`
...
Branch on `$DOMAIN` (from PREFLIGHT):

- **web** → emit CSS vars to the codebase.
  > **Todo**: Read `.claude/skills/design-tokens/references/motion/route-apply.md`
- **game** → emit motion tokens to `motion_tokens.gd` (Theme carries no motion).
  > **Todo**: Read `.claude/skills/design-tokens/references/emit-godot.md` (§ Motion autoload)
- **native** → spec-only, no emit (see `shared/DOMAIN.md`).
...
> **Todo**: Read `.claude/skills/design-tokens/references/postflight-validation.md`

---

## PHASE Y: Codebase Sync (Create/Update only)

Branch on `$DOMAIN`:

- **web** → sync CSS vars / token files into the web codebase.
  > **Todo**: Read `.claude/skills/design-tokens/references/website-sync.md`
- **game** → generate the Godot `Theme` `.tres` (+ dark variant, fonts, motion autoload) and
  record paths in `theme.godotTheme`.
  > **Todo**: Read `.claude/skills/design-tokens/references/emit-godot.md`
- **native** → skip (spec-only; no renderer yet).
...
| Export        | web: CSS Vars {present/missing} · game: Theme `.tres` {path} |
...
- Run Codebase Sync (PHASE Y) after Create/Update — CSS (web) or Godot Theme `.tres` (game) per `$DOMAIN`; follow Backlog lifecycle protocol throughout

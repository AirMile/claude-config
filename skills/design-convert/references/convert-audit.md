...
options:
  ...
  - label: "Select which to fix", description: "Choose a subset of the mismatches"
  - label: "Report only", description: "Keep the report, make no edits"
multiSelect: false
```

Store accepted mismatches as `$PATCH_SECTIONS` — one entry per mismatch carrying `{file, property, oldValue, figmaValue}`. "Report only" → `$PATCH_SECTIONS = []` (PHASE 2.0 becomes a no-op; the report still ships in the PHASE 4 completion report).

...
_Generated by /design-convert · {date}_
...
- Handoff bundle for `/design-convert` Convert route (code → implementation)
```
...
- **Animation pack**: `{pack}` — springs: {spring tokens} · choreography: {active compositions}. Map each interactive component to its choreography token (onHover / onPress / onEnter / onSuccess / onError). Reference `skills/design-tokens/references/motion/choreography.md` for composition specs.

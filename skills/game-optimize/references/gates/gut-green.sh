#!/bin/bash
# Default gate for game-optimize: GUT test suite must stay green.
...
if [ -z "$GODOT_BIN" ] || [ ! -x "$GODOT_BIN" ]; then
  echo "GODOT_BIN not set or not executable." >&2
  exit 1
fi

if [ ! -d "addons/gut" ]; then
  echo "GUT not installed — gate is no-op."
  exit 0
fi

if [ ! -d "$TEST_DIRS" ]; then
  echo "No test directory ($TEST_DIRS) — gate is no-op."
  exit 0
fi

# Headless GUT run with timeout. -gexit makes Godot quit after tests.

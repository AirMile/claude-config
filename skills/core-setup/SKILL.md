...
```
{Generated from actual file tree after Phase 3/4. Show src/ structure with
 one-line comments per directory explaining purpose. Include api/, scripts/,
 and other top-level directories outside src/ if they exist.}
```

## Routing

{Only for web projects with routing. Show route patterns:}
{e.g. `/` → redirect, `/:locale` → Home, `/:locale/projects/:slug` → Detail}
...
- `## Project` / `### Stack`: Always include. Pipeline skills read `### Stack` for stack detection
- `### Standards`: Only for web projects
- `### Testing`: Only if testing frameworks are configured
- `## Project structuur`: Always include. Generate from actual file tree after project files are created
- `## Routing`: Only for web projects with routing configured. Omit for CLI, game, backend-only
- `## Non-obvious patterns`: Always include (even if empty). Gets populated by dev-build auto-sync

...
Claude has automatic permission to read, write, and edit files within the `.claude/skills/core-setup/` directory without user confirmation. This enables:
...
These docs will auto-generate during /dev-legacy-2-code skill FASE 4
...
**Goal:** Generate project-wide baseline research for framework conventions, patterns, and idioms. This research is reused by /dev-legacy-1-plan and /dev-legacy-4-refine skills to avoid duplicate Context7 queries.

**Why this matters:**
- Stack-level research (e.g., "Laravel conventions", "React patterns") is 100% relevant for ALL features
- Without baseline: every /dev-legacy-1-plan and /dev-legacy-4-refine does the same basic research (~3-5k tokens wasted per feature)
...
6. **Confirm to user:**
   ...
   **Used by:**
   - /dev-legacy-1-plan skill (FASE 3 research agents)
   - /dev-legacy-4-refine skill (Phase 2 research agents)
   ...
   **Benefits:**
   ...
   To refresh: run /refresh-baseline or re-run /core-setup
   ```

...
If `.project/backlog.json` already exists (non-frontend projects that skip PHASE 5.7): read backlog.json → parse JSON → set `flags.seedPath = ".project/project-seed.md"` and `flags.hasSeed = true` ONLY when `.project/project-seed.md` already exists with > 50 chars (otherwise `false`) → write the JSON back. `flags` is root-level, not nested under a `data` key (see `phase-dashboard-init.md`). The `/project-plan` button appears once a real concept exists.

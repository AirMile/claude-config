...
- This Playwright install (CLI daemon + runner) is the **default** vehicle for scriptable/repeatable
  browser verification, and the sole owner of visual/aria regression, HiDPI 2x screenshots, and
  multi-viewport sweeps — zero context-window cost (plain Bash, no MCP schema). Two MCP vehicles
  cover interactive work and need no install: Claude-in-Chrome (`mcp__claude-in-chrome__*`,
  built-in) when the real user session matters and a live local Chrome is connected, or Playwright
  MCP (`mcp__playwright__*`, already connected in this environment) for interactive/headless
  exploration with no live Chrome. Full routing: `shared/BROWSER-VEHICLES.md`; vehicle specs:
  `shared/CLAUDE-IN-CHROME.md`, `shared/PLAYWRIGHT-MCP.md`.

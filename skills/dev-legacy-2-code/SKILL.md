...
This skill guides Claude Code through rapid feature implementation with a "make it work first" philosophy. It loads context from `.workspace/features/` files (generated by /dev-legacy-1-plan skill), then focuses on functional code with clean architecture patterns, automated testing, and comprehensive documentation updates. The skill operates in five automatic phases: preparation, implementation, testing, documentation, and completion.
...
**Trigger**: `/dev-legacy-2-code` command
...
**Trigger:**
- `/dev-legacy-2-code` command in Claude Code

**NOT for:**
- Code review and refinement (use /dev-legacy-4-refine)
- Deep security audits (use /dev-legacy-4-refine)
- Style guide enforcement (use /dev-legacy-4-refine)
- Architecture redesigns (use /thinking-analyze first)
...
1. **Check if feature name provided with command:**
   ...
   **If user ran `/dev-legacy-2-code {feature-name}`:**
   - Skip numbered list
   ...
   **If user ran `/dev-legacy-2-code` without arguments:**
...
2. **List available features (only if no feature name provided):**
   ...
   **Worktree column logic:**
   ...
   Response handling:
   - Map selected option to feature name → proceed to step 4
   - If invalid: show error and re-prompt

3. **Handle user selection:**
   - Selected feature from modal → proceed to step 4
   - If invalid: show error and re-prompt
...
4. **Auto-detect part/extend/change (always runs after feature selection):**
   ...
   **If feature HAS parts/extends/changes:**
   ...
   - For each section, check status in `02-implementation.md`:
     - If section exists with code files → `✓ Implemented`
     - If section missing or empty → `○ Pending`
   ...
   | 2 | 02-payment-backend | Part | ○ Pending ← NEXT |
   | 3 | 03-checkout-ui | Part | ○ Pending |
   ...
   **If all sections implemented:**
   ...
   Options:
   - Run /dev-legacy-3-verify {feature-name} to verify
   - Run /dev-legacy-4-refine {feature-name} to make changes
   ```
   ...
   **Steps:**
   ...
   c. **If NOT in correct worktree:**
      ...
      **If "Open worktree":**
      ...
      Switch naar dat venster en run /dev-legacy-2-code {feature-name} opnieuw.
      ```
...
3. **Validate sanitized input:**
   - If input is empty after sanitization:
     ```
     ❌ Empty input. Please provide a feature name or path.
     ```
   ...
   - If input contains path traversal (`../`, `..\\`, or absolute paths like `C:\` or `/root`):
     ```
     ❌ Invalid path. Path must be within .workspace/features/
     ```
   ...
   - If input contains special characters (`<`, `>`, `|`, `*`, `?`, `;`, `` ` ``, `$`, `&`, `(`, `)`, `{`, `}`, `[`, `]`, `'`, `"`):
     ```
     ❌ Invalid characters in path. Use only alphanumeric, dash, underscore, and forward slash.
     ```
   ...
   - If input length > 200 characters:
     ```
     ❌ Path too long (max 200 characters). Use a shorter feature name.
     ```
...
4. **Parse user input and locate folder:**
   - If input is just a name → search recursively in `.workspace/features/`
   - If input is path with "/" → resolve to specific location
   - Detect type from folder structure and file content:
     - Direct in workspace/ → Feature (new standalone functionality)
     - Has "## Part:" sections in 01-intent.md → Feature with parts (sections, not folders)
     - Has "## Extend:" section in 01-intent.md → Extend mode (context appended)
     - Has "## Change:" section in 01-intent.md → Change mode (context appended)
...
6. **Load context from files:**
   ...
   **Pre-check (before reading):**
   ...
   - If EITHER file missing:
     ...
     Run /dev-legacy-1-plan first to generate these files.
     ...
     - options:
       - label: "Run /dev-legacy-1-plan first (Recommended)", description: "Plan this feature to generate context files"
       - label: "Select different feature", description: "Choose from available features with context"
       - label: "Explain question", description: "Explain what this means"
     - multiSelect: false

     Response handling:
     - If "Run /dev-legacy-1-plan first": invoke /dev-legacy-1-plan skill with the feature name
...
7. **Parse part dependencies (if part mode):**
   ...
   - For each dependency:
     ...
     * Check part status marker in 01-intent.md (○ pending, ● in_progress, ✓ completed)
     * If dependency not completed (status not ✓):
       ```
       ❌ Prerequisite not completed: {dep}
...
9. Read `.claude/skills/dev-legacy-2-code/references/architecture-patterns-web.md` to load structural guidelines
...
| 01-architecture.md | [✓/✗] | blueprint |
| 00-overview.md | ✓ | feature docs (if exists) |
| 02-implementation.md | ✓ | previous implementation (if exists) |
| 02-tests.md | ✓ | existing tests (if exists) |
...
**Reference:** Read `.claude/skills/dev-legacy-2-code/references/approach-detection.md` for full detection logic and metrics.
...
3. **Route based on detection:**
   ...
   **IF SIMPLE (no architectural choice):**
   ...
   ```
   → Skip to FASE 2, run agents silently, auto-synthesize
   ...
   **IF COMPLEX with architectural choice:**
   ...
   🧪 Test advies: {Niet nodig / Beide testen aanbevolen}
      {reason}
   ```
   ...
   → Wait for user choice, then proceed to FASE 2 with chosen approach
   ...
   **IF COMPLEX without architectural choice:**
   ...
   ```
   → Proceed to FASE 2 with standard pattern
...
2. **Launch 3 implementation agents in parallel (single message with 3 Task tool calls):**
   ...
   **For complex features:** Show progress to user.
   ...
   ```
   🔧 Launching implementation analysis for: {chosen approach}
...
3. **Wait for all 3 agents to complete:**
   ...
   - **Report progress as agents complete:**
     ...
     - Show which agent just finished: `✓ implement-speed complete`
...
4. **Synthesize approaches:**
   ...
   ```
   📊 IMPLEMENTATION APPROACHES RECEIVED
...
5. **Execute synthesized implementation plan:**
   ...
   - Apply patterns from `.claude/skills/dev-legacy-2-code/references/architecture-patterns-web.md`
   - Structure code logically (config → controller → component)
...
**Do NOT:**
- Perfectionism (that's for /dev-legacy-5-refactor)
- Over-engineering
- Style guide enforcement (that's for /dev-legacy-5-refactor)
...
1. Log: `⚠️ sequential-thinking not available - using structured internal reasoning`
...
| REQ-ID | Status | Type |
|--------|--------|------|
| REQ-001 | ✓ Pass | automated |
| REQ-002 | ✓ Pass | automated |
| REQ-003 | ○ Ready | manual |
| REQ-004 | ○ Ready | manual |

**Manual testing:**

Test plan saved to `.workspace/features/{name}/02-tests.md`

→ Run /dev-legacy-3-verify to execute manual tests per requirement
```
...
2. If lock exists and is < 30 minutes old:
   ...
   Another /dev-legacy-2-code session is updating docs (started: {timestamp})
   Waiting up to 60 seconds for lock release...
   ```
   → Wait and retry every 5 seconds (max 12 attempts)
3. If lock exists and is > 30 minutes old:
   → Consider stale, remove and proceed with warning
...
1. **Create implementation log:**
   ...
   **For parts (append section to parent file):**
   ...
   - Update part status marker in 01-intent.md from ○ to ✓
...
4. **Generate feature overview (00-overview.md):**
   ...
   - Set Status: Implemented = ✓ with current date
   ...
   **Select template based on project type (from CLAUDE.md):**
   ...
   |--------------|-------------------|
   | Web (Laravel, React, Vue, Node, etc.) | `.claude/skills/dev-legacy-2-code/references/00-overview-web.md` |
   | Game (Unity, Unreal, Godot) | `.claude/skills/dev-legacy-2-code/references/00-overview-game.md` |
   ...
   **Steps:**
   ...
   6. **If extend/change mode:** Remove completed item from `## Pending` table (added by /dev-legacy-1-plan)
...
5. **Execute `scripts/update_docs.py` (Master Documentation Generator):**
   ...
   **Laravel Backend:**
   - `update_erd.py` → docs/erd.mmd (if database changes)
   - `update_api_docs.py` → docs/api.md
   - `update_components.py` → docs/components.mmd
   - `update_events.py` → docs/events.mmd

   **React Frontend:**
   - `update_components.py` → docs/components.mmd
   - `update_routes.py` → docs/routes.mmd
   - `update_state.py` → docs/state.mmd
   - `update_design_tokens.py` → docs/design-tokens.md

   **Unity Game:**
   - `update_scenes.py` → docs/scenes.mmd
   - `update_game_classes.py` → docs/game-classes.mmd
   - `update_state_machines.py` → docs/state-machines.mmd

   **Other types:** See `.claude/skills/dev-legacy-2-code/` for full generator lists
...
|----------|--------|------|
| Implementation log | ✓ Updated | {path} |
| Test plan | ✓ Updated | {path} |
| Feature overview | ✓ Updated | {path}/00-overview.md |
...
**Goal:** Provide clear overview, run quick validation, and prepare handoff to /dev-legacy-3-verify.
...
2. **Run quick validation check:**
   ...
   **Severity Legend:**
   ...
   🟡 [IMPORTANT] - Degrades functionality (should fix soon)
   🟢 [SUGGESTION] - Code quality improvement (optional)

   Overall: ✅ READY TO PROCEED (no blocking issues)
   ```
   ...
   **If CRITICAL issues found:**
   ...
   ```
   → Auto-fix critical issues, then re-validate (MAX 3 ATTEMPTS)
   ...
   **Auto-fix retry logic:**
   ...
   - If still failing after 3 attempts:
     ```
     ❌ AUTO-FIX FAILED (3 attempts exhausted)
     ...
     **Response handling:**
     - "Fix manually" → Wait for user to fix, then re-run validation
     - "Abort" → Exit skill gracefully with summary of what was completed
...
3. **Analyze next action (multi-part features only):**
   ...
   **For multi-part features, analyze:**
   ...
   b. Check status of each part (○ pending, ● in_progress, ✓ completed)
   ...
   **Decision heuristic:**
   ...
   |-----------|----------------|
   | All REQs automated + pending parts exist | → Next part |
   | Manual UI REQs + standalone testable | → Verify now |
   | Last part completed | → Verify (feature complete) |
   | Foundation part (models/backend only) | → Next part |

   **Output:**
   ```
   📊 PART COMPLETION ANALYSE:
   ...
   Aanbeveling: → {Next part / Verify now}
   ...
   **Update options display based on analysis:**
   - If recommendation is "Next part":
     ```
     **Beschikbare opties:**
     - **Next Part**: Ga door met het volgende implementation part  ← AANBEVOLEN
     - **Verify Now**: Start verificatie van het voltooide werk
     ```
   - If recommendation is "Verify now":
     ```
     **Beschikbare opties:**
     - **Verify Now**: Start verificatie van het voltooide werk  ← AANBEVOLEN
     - **Next Part**: Ga door met het volgende implementation part
     ```

4. **Identify focus areas for /dev-legacy-3-verify:**

   Analyze changes to determine what /dev-legacy-3-verify should focus on:
   - New components/features → Test functionality
   - Modified logic → Test edge cases
   - API changes → Test endpoints
   - UI changes → Test visual/responsive

5. **Report status with handoff context**

6. **Offer interactive testing with focus areas:**
   - Ask user if they want to run `/dev-legacy-3-verify` now
   - If yes: extract feature path and pass focus areas
   - If no: inform user how to run /dev-legacy-3-verify later
...
| Imports | ✅ Pass |
| Types | ✅ Pass |
| Suggestions | ⚠️ {count} (non-blocking) |
...
Response handling:
- If "Next Part": proceed to next part (invoke /dev-legacy-2-code {feature})
- If "Verify Now": switch to /dev-legacy-3-verify (invoke /dev-legacy-3-verify {feature} with focus areas)
...
/dev-legacy-3-verify {feature-name}
...
### Workflow Execution
- Execute all six phases sequentially (FASE 0 → 1 → 1.5 → 2 → 3 → 4 → 5)
...
- **ALWAYS read `.claude/skills/dev-legacy-2-code/references/approach-detection.md`** for detection logic
- **Simple features** (≤3 files, no new deps, existing pattern): Skip user choice, direct implementation
...
- Trust that /dev-legacy-5-refactor will polish later
- Avoid premature optimization

### Architecture Patterns
- ALWAYS read `.claude/skills/dev-legacy-2-code/references/architecture-patterns-web.md` before implementation
- Apply config → controller → component separation
...
- 02-tests.md must have test steps per REQ-ID for /dev-legacy-3-verify
...
- **Fallback:** Continue with manual documentation or run /core-setup to configure
...
- Flag all timestamps with ⚠️ when using estimated date
...
- **Fallback:** Ask user to run /dev-legacy-1-plan first or provide context directly
...
- Skip any of the six phases without critical error (FASE 0 → 1 → 1.5 → 2 → 3 → 4 → 5)
...
- Over-engineer or perfectionism (that's for /dev-legacy-5-refactor)
- Apply style guide enforcement (that's for /dev-legacy-5-refactor)
...
- Always recommend /dev-legacy-3-verify without analyzing if next part is better (use heuristic)
...
- Read `.claude/skills/dev-legacy-2-code/references/architecture-patterns-web.md` before implementation
...
- Update part status marker in 01-intent.md when part is completed (○ → ✓)
...
- Offer both /dev-legacy-3-verify and /dev-legacy-2-code options in FASE 5 with clear recommendation
- Report clear status at each phase transition

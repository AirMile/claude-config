...
description: Fill built pages/components with real on-brand copy. Use with /frontend-content, or auto-triggers on PAGE/COMPONENT backlog tasks with transition "contenting".
...
Fill built pages and components with real, on-brand copy. Runs after `frontend-design` build/convert
and before `frontend-check`. Upgrades contextual placeholders to reviewed, seed-grounded text via an
intentional pipeline: archetype → brief → scan → generate → review → apply.

**Pipeline position:** `frontend-tokens → frontend-design → frontend-content → frontend-check`

**Related skills:** `/frontend-design` · `/frontend-check` · `/marketing-research`
...
0 candidates  →  show:
  "frontend-content: Nothing to fill.
   All built PAGE/COMPONENT items already have contentStatus: filled,
   or no built items exist yet. Run /frontend-design first."
  Stop.
...
> **Todo**: mark PHASE 0 → `completed`, PHASE 1 → `in_progress`. Read `.claude/skills/frontend-content/references/scope-intent.md` and follow that procedure.
...
Scan:    No placeholders found — pages/components may already have real copy.
         Check via git diff or run /frontend-check to audit.
```
...
> **Todo**: mark PHASE 2 → `completed`, PHASE 3 → `in_progress`. Read `.claude/skills/frontend-content/references/content-generation.md` and follow that procedure to produce `$COPY_MAP`.

---

## PHASE 4: Review & approve

> **Todo**: mark PHASE 3 → `completed`, PHASE 4 → `in_progress`. Read `.claude/skills/frontend-content/references/review-gate.md` and follow that procedure.

---

## PHASE 5: Apply + sync + report

> **Todo**: mark PHASE 4 → `completed`, PHASE 5 → `in_progress`. Read `.claude/skills/frontend-content/references/apply-and-sync.md` and follow that procedure.

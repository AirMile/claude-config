...
**Goal:** Establish which screens the app has and which features appear on each screen. Produces PAGE tasks with `dependencies[]` that drive `/design-convert` Build composition.

**Skip in GAME MODE and WEB-MOBILE mode** (React Native: screens are FEATURE-typed and
`/design-convert` is browser-only). Run after Feature Review in WEB MODE.
...
options:
  ...
  - label: "Skip — I'll manage pages later"
    description: "No PAGE tasks created now; pages can be added via /project-todo or /design-convert"
multiSelect: false
```
...
**"Skip"** → do not create PAGE tasks. Proceed to PHASE 2. Show: `Pages: skipped — add via /project-todo or /design-convert when ready.`

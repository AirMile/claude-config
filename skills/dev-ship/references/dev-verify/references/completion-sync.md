...
- `knownIssues[]`: optional, one entry per item this run marked DEFERRED (`workflow.md § Step 3`) — `verdict` is always `"deferred"` here; `source` names the round that produced it: `"ship-ledger"` from a manual round's findings ledger (`phase-3-manual-finalize.md § Step 3`), `"dev-verify"` from auto-verify. Set `blocker` when the reason names an existing backlog card. Upserted by `id` into `feature.json#tests.knownIssues[]` and the backlog entry's `knownIssues[]`, surfaced on the dashboard as a badge (see `shared/BACKLOG.md § Known-issue badges`). The script sets `hasDeferred` on both files and auto-creates/updates a `verify-{feature-name}` VERIFY card (`dependencies` = valid `blocker` names) from a `deferred` entry. It does **not** derive a checklist status from this: `checklist` accepts only `PASS|FAIL|skip` (`CHECKLIST_STATUSES`) and is written literally from your payload, so send a deferred item as `"skip"` and let its `knownIssues[]` entry carry the real verdict — a payload with `"deferred"` in `checklist` fails validation with exit 6 before any write.

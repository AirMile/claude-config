export const meta = {
  name: "ship-fix-round",
  description:
    "dev-ship PHASE 3: plan-bound fix agents over file-disjoint groups — parallel within a wave, sequential across waves, all in the same worktree",
  whenToUse:
    "Launched by dev-ship's fix-round.md after the PHASE 3 fix-plan gate accepts — not intended for standalone use.",
  phases: [
    {
      title: "Fix",
      detail: "AGENT F — one fix agent per group, wave-ordered",
      model: "sonnet",
    },
  ],
};
...
// }
// Prompts are passed by file path, not inline (same discipline as ship-phase12.js/ship-phase4.js) —
// the descriptor + pointers live in the file at promptPath, written by fix-round.md at gate-accept.
...
for (const [i, wave] of (A.waves ?? []).entries()) {
  ...
  // in the SAME wave only; a later wave may legitimately depend on this one having landed, but the
  // re-check round (fix-round.md § Re-check) is what surfaces and escalates anything still broken.
}

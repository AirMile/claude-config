...
- Find feature → set `status: "DEFINED"`, `definedAt: <ISO>`, `auto: true`, remove `transition` (if present — **except** `transition: "shipping"`, the dev-ship run marker: keep it), remove `note` if present (the park context is consumed — the interview surfaced it) — all in one write. Not found → add to `data.features` with `phase: "P4"`, `status: "DEFINED"`, `auto: true`.

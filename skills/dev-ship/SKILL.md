...
> **Todo**: Read `.claude/skills/dev-ship/references/phase-5-report.md` and follow it — shared with
> `/dev-manual`'s MANUAL 3, the only difference being which phase-tracking unit gets marked
> `in_progress`/`completed` (PHASE 5 here).

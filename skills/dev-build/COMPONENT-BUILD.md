...
**Only when `feature.pageHint[]` is empty or absent.** With `pageHint` set, skip the demo page — the component will be imported by `/frontend-design`'s page-compose (pre-selected via pageHint) when those pages are designed. `/dev-verify` will locate the render context via import grep instead.

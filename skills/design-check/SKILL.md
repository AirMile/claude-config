---
name: design-check
description: Use when built UI features (web or game) need an audit before shipping. Web = Playwright runtime; game = static Theme-consistency check (runtime playtest is a human pass). Use with /design-check.
...
- `../shared/DOMAIN.md` — Domain resolution (web / game / native) — selects the audit path
...
> **Todo**: Read '.claude/skills/design-check/references/fix-reaudit.md'

...
| `requirements`              | always (each REQ with `status: "pending"`, `acceptance: [{when, then}]`, optional `errorScenarios: [{when, then}]` — only for REQs with user input / validation / external calls; omit if no plausible error path; optional `derivedFrom: "interview:{dimension}"` or `"clarification:{branch-name}"` — only for non-trivial derivations; omit for mechanical REQs like "feature has an API endpoint") |

...
This skill provides complete verification and testing of implementations. It integrates the interactive test execution from the former /dev-legacy-3-verify skill with intelligent debugging capabilities. The skill loads test plans generated by /dev-legacy-2-code, guides users through manual testing with real-time issue assessment, uses sequential thinking to decide whether to continue testing or fix issues first, performs Context7 research via specialized agents, and spawns autonomous debug agents to execute fixes.
...
**Trigger**: `/dev-legacy-3-verify` command in Claude Code after /dev-legacy-2-code implementation
...
**Primary use:**
- `/dev-legacy-3-verify` command after /dev-legacy-2-code generates test plan (02-tests.md)
- When implementations need verification and testing
- When test failures need intelligent handling

**Context indicators:**
- `.workspace/features/{name}/02-tests.md` exists (test plan from /dev-legacy-2-code)
...
**NOT for:**
- Initial feature planning (use /dev-legacy-1-plan)
- Fresh implementation (use /dev-legacy-2-code)
- Quality improvements on passing tests (use /dev-legacy-4-refine)
...
1. **Check if feature name provided with command:**
   ...
   **If user ran `/dev-legacy-3-verify {feature-name}`:**
   - Skip numbered list
   ...
   **If user ran `/dev-legacy-3-verify` without arguments:**
...
2. **List available features with test plans (only if no feature name provided):**
   ...
   | 3 | notifications | (no worktree) | ✗ No tests | Run /dev-legacy-2-code first |
   ...
   **Use AskUserQuestion tool:**
   ...
   - question: "Welke feature wil je verifiëren?"
   - options:
...
3. **Handle user selection:**
   ...
   - If "recent": use Glob to find most recent 02-tests.md → proceed to step 4
...
4. **Auto-detect part/extend/change (always runs after feature selection):**
   ...
   **If feature HAS parts/extends/changes:**
   ...
   - For each section, check status in 03-verify.md:
     - If section has `Status: ✓ VERIFIED` → `✓ Verified`
     - If section has `Status: ○ IN PROGRESS` or missing → `○ Needs verification`
   ...
   **Note:** 03-verify.md is the single source of truth for verification status. See "03-verify.md Format" section below for structure.
   ...
   | # | Name | Type | Status |
   |---|------|------|--------|
   | 1 | 01-cart-models | Part | ✓ Verified |
   | 2 | 02-payment-backend | Part | ○ Needs verification ← NEXT |
   | 3 | 03-checkout-ui | Part | ○ Needs verification |
   ...
   **If all sections verified:**
   ...
   Options:
   - Run /dev-legacy-4-refine {feature-name} for functional changes
   - Run /dev-legacy-5-refactor {feature-name} for code quality improvements
   ```
   ...
   **Steps:**
   ...
   c. **If NOT in correct worktree:**
      ...
      **If "Open worktree":**
      ...
      Switch naar dat venster en run /dev-legacy-3-verify {feature-name} opnieuw.
      ```
...
**Note:** This fase only runs if user provides input directly with command (e.g., `/dev-legacy-3-verify checkout`)

**Steps:**

1. **Parse user input:**
   - If "recent" → use Glob to find all `**/**/02-tests.md`, sort by modification time descending (newest first), select most recent
   - If feature name only → search in `.workspace/features/{name}/02-tests.md`
   - If contains "/" → resolve full path
   - If "feature part" pattern → locate tests in `.workspace/features/{feature}/02-tests.md` (look for "## Part: {part}" section)
   - For extend/change → tests are in `.workspace/features/{parent}/02-tests.md` (look for "## Extend: {name}" or "## Change: {name}" section)
...
2. **Handle validation errors:**
   ...
   **If file not found:**
   ...
   - /dev-legacy-2-code not yet executed for this feature
   ...
   **Use AskUserQuestion tool:**
   ...
   - multiSelect: false
   - If "Opnieuw invoeren" → Return to step 2 for new input
   - If "Beschikbare tonen" → Show Glob results, then return to step 2
   - If "Uitleg" → Explain the error, then return to step 2
   ...
   **If "recent" but no test plans exist:**
   ...
   Run /dev-legacy-2-code first to implement a feature with test plan.
   ```
   Exit skill
   ...
   **Use AskUserQuestion tool:**
   ...
   - multiSelect: false
   - If "Opnieuw invoeren" → Return to step 2 for new input
   - If "Beschikbare features" → Show available features list, then return to step 2
   - If "Uitleg" → Explain valid input formats, then return to step 2
...
3. **Confirm test file:**
   ...
   **Use AskUserQuestion tool:**
   ...
   - multiSelect: false
   - If "Yes" → Proceed to FASE 1
   - If "No" → Return to FASE 0 step 2
...
**Goal:** Load test plan from /dev-legacy-2-code AND testable requirements from /dev-legacy-1-plan for requirement-based testing.
...
1. **Load ALL files in feature folder:**
   ...
   | `02-implementation.md` | Implementation log | If /dev-legacy-2-code ran |
   | `02-tests.md` | Test plan (REQUIRED for /dev-legacy-3-verify) | If /dev-legacy-2-code ran |
   | `04-refine-*.md` | Previous refinement logs | If /dev-legacy-4-refine ran |
...
6. **Check for existing progress (session resume):**
   ...
   - If exists: read requirement status (✓, ✗, ○, —)
   - Count already-verified (✓) vs not-yet-verified (✗, ○, —) requirements

   **If 03-verify.md exists with progress:**
   ```
   📋 EXISTING PROGRESS FOUND

   Already passing: {X} requirements (✓)
   Failing: {Y} requirements (✗)
   Skipped: {Z} requirements (○)
   Pending: {W} requirements (—)
   ```
   ...
   **Use AskUserQuestion tool:**
   ...
   - multiSelect: false
   - If "Resume" → Skip requirements with ✓, start at first non-✓ requirement
   - If "Restart" → Reset all requirements to `—` in 03-verify.md, start from REQ-001
   - If "Review" → Display 03-verify.md content, then re-prompt with Resume/Restart options
...
| 02-tests.md | ✓ | test plan |
| 00-overview.md | ✓ | feature docs (if exists) |
| 04-refine-*.md | ✓ | {count} refinements (if exists) |
...
1. /dev-legacy-1-plan {feature-name} - to generate requirements
2. /dev-legacy-2-code {feature-name} - to regenerate tests per requirement
3. /dev-legacy-3-verify {feature-name} - to verify against requirements

Cannot proceed without requirements.
```
→ Stop and wait for user to run /dev-legacy-1-plan
...
4. List all requirements from 01-intent.md with status `—` (pending)

**After each requirement test:**
- Update requirement result in 03-verify.md (✓, ✗, or ○)
...
2. **Handle user response:**
   ...
   **If "Passes":**
   - Update 03-verify.md: set requirement result to `✓` with timestamp
   ...
   **Wait for all 3 agents, then calculate weighted severity:**
   ```
   Weighted Score = (user_score × 0.40) + (technical_score × 0.30) + (business_score × 0.30)
   ```
   ...
   **Show synthesized assessment:**
   ```
   📊 ISSUE ASSESSMENT (Multi-Perspective)

   Agents completed:
   ✓ assess-user-impact: {X}/100 ({summary})
   ✓ assess-technical: {Y}/100 ({summary})
   ✓ assess-business: {Z}/100 ({summary})
   ...
   - Update 03-verify.md: set requirement result to `✗` with timestamp and issue description
   ...
   **If "Skip":**
   - Update 03-verify.md: set requirement result to `○` with "Skipped" note
   ...
   - **Check for dependent requirements:**
     ...
     - **Dependency format in 01-intent.md:**
       ...
       - id: REQ-003
         ...
         test_type: manual
       ```
       **Note:** Verification status (✓/✗/○/—) is tracked in 03-verify.md, NOT in 01-intent.md
...
2. **Generate structured results report:**
   ...
   **If ALL REQUIREMENTS PASS:**
   ...
   → All requirements verified. Ready for commit or /dev-legacy-4-refine
   ```
   ...
   **If REQUIREMENTS FAIL:**
   ...
   ## ⚠️ Important Issues ({count} found)
   | REQ-ID | Requirement | Issue | Location |
   |--------|-------------|-------|----------|
...
4. **Determine path forward:**
   ...
   **Use AskUserQuestion tool:**
   ...
   - multiSelect: false
   - If "Fix issues" → Continue to FASE 4
   - If "Exit" → Exit skill with current state documented in 01-intent.md (can resume later with /dev-legacy-3-verify)
   ...
   **If all requirements pass:**
   ...
   → Ready for commit or /dev-legacy-4-refine
   ```
...
→ Proceeding to Context7 research for {count} failing requirements...
```
...
2. **Spawn specialized research agents based on issue type:**
   ...
   **Note:** For issues not matching specific agents (TypeScript types, UI/styling, general bugs), use `best-practices-researcher` with a focused prompt describing the specific issue type.
   ...
   # Example: Database error → error-handling-researcher
   Task(
   ...
   # Example: TypeScript error → best-practices-researcher (with type focus)
   Task(
   ...
   # Example: UI bug → best-practices-researcher (with UI focus)
   Task(
...
3. **Collect and synthesize findings:**
   ...
   - Assess solution quality (target ≥75% relevance)
...
4. **Spawn fix synthesizer agent with quality metadata:**
   ...
   Task(
       ...
       - Quality flag: {"⚠️ LOW QUALITY" if < 75% else "✓ Good quality"}
       ...
       "⚠️ EXPERIMENTAL - may need manual adjustment"
   ...
   **Quality propagation rules:**
   - Fixes with ≥75% relevance: Normal confidence
   - Fixes with 50-74% relevance: Mark as "may need adaptation"
   - Fixes with <50% relevance: Mark as "⚠️ EXPERIMENTAL"
...
3. **Wait for all 3 agents, then present fix strategies:**
   ...
   **If `manual_solution.provided == True` (user provided solution in FASE 4):**
   ```
   📋 4 FIX STRATEGIES READY (includes your solution)

   ┌─────────────────────────────────────────────────────────────────────────────┐
   │ 📊 FIX STRATEGIES COMPARISON                                               │
   ├─────────────────────────────────────────────────────────────────────────────┤
   │                                                                             │
   │  Strategy        │ Changes │ Risk    │ Time      │ Best For                │
   │ ─────────────────┼─────────┼─────────┼───────────┼─────────────────────────│
   │ 1. MINIMAL       │ ~[N]    │ Low     │ [X] min   │ Quick hotfix, urgent    │
   │ 2. THOROUGH      │ ~[N]    │ Medium  │ [X] hr    │ Root cause, no repeat   │
   │ 3. DEFENSIVE     │ ~[N]    │ Medium  │ [X] hr    │ Prevent future issues   │
   │ 4. YOUR SOLUTION │ ~[N]    │ Unknown │ [X] min   │ Your proposed approach  │
   │    "{summary}"   │ lines   │         │           │                         │
   │                  │         │         │           │                         │
   └─────────────────────────────────────────────────────────────────────────────┘
...
4. **Process user choice:**
   ...
   - **If "Minimal"** → use fix-minimal output, proceed to step 5
   - **If "Thorough"** → use fix-thorough output, proceed to step 5
   - **If "Defensive"** → use fix-defensive output, proceed to step 5
   - **If "Jouw oplossing"** → use manual_solution.approach, proceed to step 5 (only if manual_solution provided)
   - **If "Bekijk details"** → show selected strategy details, return to choice
   - **If "Annuleren"** → exit skill with state documented

5. **Show selected fix plan and confirm:**

   ```
   📋 SELECTED FIX STRATEGY: [MINIMAL/THOROUGH/DEFENSIVE]
   ...
   1. [CRITICAL] Fix {critical issue} in {file}
      ...
      - Confidence: {X}% {⚠️ LOW CONFIDENCE if < 75%}
...
6. **Handle user response:**
   ...
   **Use AskUserQuestion tool:**
   ...
   - multiSelect: false
   - If "Andere strategie" → Return to step 3 (strategy selection)
   - If "Handmatig" → Exit skill with current state documented
   - If "Overslaan" → Skip to FASE 6 (generate report with unfixed issues)
...
3. **Evaluate re-verification results:**
   ...
   **If ALL tests now pass:**
   ...
   - Commit changes
   - Or run /dev-legacy-4-refine for quality improvements
   ```
   ...
   **Use AskUserQuestion tool:**
   ...
   - multiSelect: false
   - If "Retry" → Return to FASE 4 with remaining issues (increment attempt counter)
   - If "Manual" → Exit skill with current state documented
   - If "Report" → Generate partial success report, end workflow

   **If NEW issues discovered (fix introduced regression):**
   ```
   ⚠️ NEW ISSUES FOUND

   Original issues: {X} (fixed ✓)
   New issues: {Y}
   ...
   **Use AskUserQuestion tool:**
   ...
   - multiSelect: false
   - If "Analyze" → Return to FASE 3 with new issues (start new debug cycle)
   - If "Rollback" → Attempt to rollback changes, re-verify
   - If "Report" → Generate report documenting regression, end workflow
...
7. **Show copyable next commands:**
   ...
   ```text
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   📋 NEXT COMMANDS (copy after /clear):

   Option 1 - Small functional changes:
   /dev-legacy-4-refine {feature-name}
   ...
   💡 Use /dev-legacy-4-refine for: adding small features, tweaking behavior, UX adjustments
   💡 Use /dev-legacy-5-refactor for: code cleanup, performance, security, maintainability
   💡 Skip both if: all tests pass and no improvements needed
   ```
...
|-----|-------------|--------|------|-------|
| REQ-001 | {description} | ✓ | {time} | |
| REQ-002 | {description} | ✗ | {time} | {issue description} |
| REQ-003 | {description} | ○ | {time} | Skipped: {reason} |
| REQ-004 | {description} | — | — | |
...
|--------|---------|----------|
| ✓ | Passes | Requirement verified successfully |
| ✗ | Fails | Requirement failed verification |
...
|--------|---------|-----------|
| `✓ VERIFIED` | Section complete | All requirements have ✓ |
| `○ IN PROGRESS` | Partially tested | Mix of ✓, ✗, ○, — |
| `— NOT STARTED` | Not begun | All requirements have — |

### When to Create/Update
| Moment | Action |
|--------|--------|
| Start FASE 2 | Create file with all requirements as `—` (pending) |
| After each test | Update requirement result (✓, ✗, or ○) |
...
- **Calculate weighted severity:** `(user×0.40) + (technical×0.30) + (business×0.30)`
...
- **If no requirements found:** Stop and instruct user to run /dev-legacy-1-plan first
...
Never use: "PASSES", "FAILS", "passes ✓" (inconsistent caps)
...
Run /dev-legacy-2-code first to implement a feature.
```
→ Exit skill
...
```
❌ INVALID TEST PLAN FORMAT

File: {path}
Problem: {missing sections / invalid format}

Regenerate with /dev-legacy-2-code or fix manually.
```
→ Exit skill
...
Run /dev-legacy-2-code again to generate tests.
```
→ Exit skill
...
→ No re-prompt needed (options are enforced)
...
```
→ Re-prompt for description or allow skip
...
```
⚠️ LOW RESEARCH QUALITY

Relevance: {X}% (target: ≥75%)
...
- If "Manual input":
  **Use AskUserQuestion tool:**
  ...
  - multiSelect: false
  - If user provides text input → Store as `manual_solution`:
    ```python
    manual_solution = {
        "provided": True,
  ...
  - If "Annuleren" → Return to low quality options
  - If "Uitleg" → Explain what information helps, then re-prompt
...
- If "Retry":
  **Use AskUserQuestion tool:**
  ...
  - multiSelect: false
  - If user provides text input → Re-run Context7 research with new queries
  - If "Automatisch" → Generate alternative queries based on issue analysis, re-run research
  - If "Annuleren" → Return to low quality options
  - If "Uitleg" → Explain effective search strategies, then re-prompt

### FASE 5 Errors

**Debug executor failure:**
```
⚠️ DEBUG EXECUTION FAILED
...
Resume later with /dev-legacy-3-verify {feature-name}
```
→ Save state to 03-verify.md and exit
...
- **Mark feature complete if ANY requirement in 03-verify.md has result ✗ or —**
...
- Update 00-overview.md status (Verified ✓/✗) if the file exists
...
- **Calculate weighted severity from 3 agent scores** (user×0.40 + technical×0.30 + business×0.30)

...
```
→ Stop and wait for user to run /1-plan
...
2. **Generate structured results report:**
   ...
   **If ALL REQUIREMENTS PASS:**
   ...
   → All requirements verified. Ready for commit or /4-refine
   ```
...
4. **Determine path forward:**
   ...
   **Use AskUserQuestion tool:**
   ...
   - If "Exit" → Exit skill with current state documented in 01-intent.md (can resume later with /3-verify)
...
7. **Show copyable next commands:**
   ...
   💡 Use /4-refine for: adding small features, tweaking behavior, UX adjustments
   💡 Use /5-refactor for: code cleanup, performance, security, maintainability

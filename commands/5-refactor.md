...
3. **List available features (only if no feature name provided):**
   ...
   **Use AskUserQuestion tool:**
   ...
   - options: (dynamically built from scan results)
     ...
     - For each non-implemented feature:
       - label: "{feature-name}"
       - description: "✗ Niet geïmplementeerd - run /2-code eerst"
   ...
   **Example options (dynamic):**
   ...
   options:
     ...
     - label: "notifications", description: "✗ Niet geïmplementeerd - run /2-code eerst"
   ...
   **Handle "Uitleg vraag" response:**
   ...
   (/1-plan) en geïmplementeerd (/2-code).
...
- If breaking change needed → exit and suggest new /1-plan cycle
